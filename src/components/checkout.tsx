...
export function Checkout() {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<ShippingInput>({ resolver: zodResolver(shippingSchema), defaultValues: { province: 'Punjab' } });
    ...
    if (!ready)
        return <section className="section">Loading checkout...</section>;
    if (!cart.length)
    ...
    const fields: [
        ...
        string,
        string?
    ][] = [['customer_name', 'Full name'], ['phone', 'Mobile number', 'tel'], ['alternate_phone', 'Alternative mobile (optional)', 'tel'], ['email', 'Email (optional)', 'email'], ['city', 'City'], ['address', 'Complete delivery address'], ['landmark', 'Nearby landmark (optional)'], ['postal_code', 'Postal code (optional)'], ['notes', 'Delivery notes (optional)']];
    return <section className="section"><p className="eyebrow">NO ACCOUNT. JUST GOOD CHOICES.</p><h1>Checkout.</h1><div className="checkout-grid"><form onSubmit={handleSubmit(review)} onChange={() => { setQuote(null); setShipping(null); }}><h2>Delivery details</h2><div className="form-grid">{fields.map(([name, label, type]) => <label key={name} className={['address', 'notes'].includes(name) ? 'full' : ''}>{label}{['address', 'notes'].includes(name) ? <textarea {...register(name)} rows={3}/> : <input type={type || 'text'} autoComplete={name === 'customer_name' ? 'name' : name === 'phone' ? 'tel' : name === 'email' ? 'email' : undefined} {...register(name)}/>}<small className="error">{errors[name]?.message}</small></label>)}<label>Province<select {...register('province')}>{['Punjab', 'Sindh', 'Khyber Pakhtunkhwa', 'Balochistan', 'Islamabad', 'Azad Kashmir', 'Gilgit-Baltistan'].map(p => <option key={p}>{p}</option>)}</select></label></div><div className="payment"><strong>Cash on delivery</strong><p>Pay for your order when it reaches your door.</p></div><button className="button gold" disabled={busy}>{busy ? 'Checking...' : 'Verify availability & review order'} ?</button></form><aside className="order-summary"><h2>Your order</h2>{(quote?.items || cart).map(i => <p key={i.variantId}><span>{i.name}<small>{i.size && `EU ${i.size}  /  `}{i.color}  x  {i.quantity}</small></span><span>{money(i.price * i.quantity)}</span></p>)}{quote && reviewed === signature ? <><p><span>Subtotal</span>{money(quote.subtotal)}</p><p><span>Delivery</span>{money(quote.delivery)}</p><p className="total"><span>Total</span>{money(quote.total)}</p><p className="verified">Prices and availability checked. Ready for your review.</p><small>By placing your order you agree to our <Link href="/terms">terms</Link> and <Link href="/privacy">privacy policy</Link>.</small><button className="button gold wide" disabled={busy} onClick={place}>{busy ? 'Placing order...' : 'Place order  /  ' + money(quote.total)}</button></> : <p>Complete your delivery details to verify stock, prices, and delivery charges.</p>}<p className="error" role="alert">{error}</p><small>Your order is created only after inventory is confirmed. No online payment required.</small></aside></div></section>;
}
type TrackingResult = {
    order_number: string;
    order_status: string;
...
export function Tracking() { const [result, setResult] = useState<TrackingResult | null>(null); const [error, setError] = useState(''); const [busy, setBusy] = useState(false); return <section className="section narrow"><p className="eyebrow">FROM OUR DOOR TO YOURS</p><h1>Track your order.</h1><p>Enter the order number from your receipt and the mobile number used at checkout.</p><form className="tracking-form" onSubmit={async (e) => { e.preventDefault(); const data = new FormData(e.currentTarget); setBusy(true); setResult(null); setError(''); try {
    ...
    setBusy(false);
} }}><label>Order number<input name="orderNumber" required placeholder="TS-20260913-XXXXXXXXXX"/></label><label>Mobile number<input name="phone" type="tel" required placeholder="03XXXXXXXXX"/></label><button className="button gold" disabled={busy}>{busy ? 'Checking...' : 'Find my order ?'}</button></form><p className="error" role="alert">{error}</p>{result && <div className="order-summary" role="status"><p className="eyebrow">{result.order_number}</p><h2 className="capitalize">{result.order_status}</h2><p>{new Date(result.created_at).toLocaleDateString()}</p>{result.order_items.map((i, n) => <p key={n}>{i.product_name}  x  {i.quantity}</p>)}<p className="total">Total {money(result.total)}</p></div>}</section>; }
